...
final class Scheduler implements SchedulerInterface
{
    ...
    public function getDueTasks(): TaskListInterface
    {
        ...
        $dueTasks = $this->transport->list()->filter(fn (TaskInterface $task): bool => (new CronExpression($task->getExpression()))->isDue($synchronizedCurrentDate, $task->getTimezone()->getName()) && $synchronizedCurrentDate->format('Y-m-d h:i') !== $task->getLastExecution()->format('Y-m-d h:i'));
